...
include $(VISIONSOM_COMMON_DEVICE_PATH)/BoardConfigCommon.mk

BUILD_TARGET_FS ?= ext4
TARGET_USERIMAGES_USE_EXT4 := true

TARGET_RECOVERY_FSTAB = $(VISIONSOM_COMMON_DEVICE_PATH)/fstab.freescale

# Support gpt
ifeq ($(TARGET_USE_DYNAMIC_PARTITIONS),true)
  BOARD_BPT_INPUT_FILES += device/fsl/common/partition/device-partitions-13GB-ab_super.bpt
  ADDITION_BPT_PARTITION = partition-table-14GB:device/fsl/common/partition/device-partitions-13GB-ab_super.bpt \
  ...
  ifeq ($(IMX_NO_PRODUCT_PARTITION),true)
    ...
    ADDITION_BPT_PARTITION = partition-table-14GB:device/fsl/common/partition/device-partitions-13GB-ab-no-product.bpt \
                             partition-table-28GB:device/fsl/common/partition/device-partitions-28GB-ab-no-product.bpt
  else
    BOARD_BPT_INPUT_FILES += device/fsl/common/partition/device-partitions-13GB-ab.bpt
    ADDITION_BPT_PARTITION = partition-table-14GB:device/fsl/common/partition/device-partitions-13GB-ab.bpt \
...
BOARD_PREBUILT_DTBOIMAGE := out/target/product/visioncb_8mm_adv_dsi/dtbo-imx8mm.img
TARGET_BOARD_DTS_CONFIG := imx8mm:visionsom-8mm-cb-adv-mipi7.dtb

BOARD_SEPOLICY_DIRS := $(VISIONSOM_COMMON_DEVICE_PATH)/sepolicy

ifeq ($(PRODUCT_IMX_DRM),true)
BOARD_SEPOLICY_DIRS += \
       $(VISIONSOM_COMMON_DEVICE_PATH)/sepolicy_drm
endif
